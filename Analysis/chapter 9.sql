...
COPY meat_poultry_egg_inspect FROM 'C:\Users\thlok\Documents\3. SQL Module\Analysis\MPI_Directory_by_Establishment_Name.csv' WITH (FORMAT CSV, HEADER, DELIMITER ',');
...
--finds another companies in the same city
SELECT est_number,
       company,
       city,
       st,
...
-- Check number of records: checks if both tables are identical
SELECT 
...
-- Listing 9-13: Creating and filling the company_standard column (ensuring there are no inconsistencies)
...
SELECT company, company_standard -----run to check if data has been cleaned up
FROM meat_poultry_egg_inspect
WHERE company LIKE 'Armour%';

UPDATE meat_poultry_egg_inspect ----cleans up the data
SET company = company_standard;

---run 87 to 106 to check the result
...
UPDATE meat_poultry_egg_inspect
SET zip = '00' || zip ----concatinate, add 2 zeros infront of zip code if it has 3 digits to bring ot to 5
...
COPY state_regions
FROM 'C:\Users\thlok\Documents\3. SQL Module\Analysis\state_regions.csv'
...
-- Start transaction and perform update: (its like a safeguuard for data because you can roll back)
START TRANSACTION;
